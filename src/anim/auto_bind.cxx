...
typedef pset<Character *> Characters;
typedef pmap<string, Characters> Parts;
 ...
 */
static void
bind_anims(const Characters &parts, const AnimBundles &anims,
           AnimControlCollection &controls,
           int hierarchy_match_flags) {
  Characters::const_iterator pbi;
...
r_find_bundles(PandaNode *node, Anims &anims, Parts &parts) {
  ...
  } else if (node->is_of_type(CharacterNode::get_class_type())) {
    CharacterNode *bn = DCAST(CharacterNode, node);
    Character *bundle = bn->get_character();
    parts[bundle->get_name()].insert(bundle);
    //int num_bundles = bn->get_num_bundles();
    //for (int i = 0; i < num_bundles; ++i) {
    //  Character *bundle = bn->get_bundle(i);
    //  parts[bundle->get_name()].insert(bundle);
    //}
  }
...
auto_bind(PandaNode *root_node, AnimControlCollection &controls,
  ...
  while (ai != anims.end() && pi != parts.end()) {
    if ((*ai).first < (*pi).first) {
      ...
      if (hierarchy_match_flags & Character::HMF_ok_wrong_root_name) {
        AnimBundles::const_iterator abi;
