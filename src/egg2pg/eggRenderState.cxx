...
find_pmat(EggPrimitive *egg_prim) {

  // First see if the material has a corresponding .pmat.
  if (egg_prim->has_material()) {
    auto it = _loader._egg_material_pmats.find(egg_prim->get_material());
    if (it != _loader._egg_material_pmats.end()) {
      // Use this one.
      return (*it).second;
    }
  }

  // If we didn't have a material or a corresponding .pmat for our material,
  // check to see if any of the textures do.
  for (int i = 0; i < egg_prim->get_num_textures(); i++) {
    auto it = _loader._egg_texture_pmats.find(egg_prim->get_texture(i));
    if (it != _loader._egg_texture_pmats.end()) {
      // Use this one.
      return (*it).second;
    }
  }

  // No .pmat to use for this primitive.
  return nullptr;
}
...
fill_state(EggPrimitive *egg_prim) {
  ...
  // Try to find a .pmat material based on the EggTextures or EggMaterial
  // referenced by the primitive.
  CPT(RenderAttrib) pmat_material = find_pmat(egg_prim);
  ...
  // If we don't have a .pmat material, all of the textures will be applied to
  // the render state through TextureAttrib.
  if (pmat_material == nullptr) {
...
get_material_attrib(const EggMaterial *egg_mat, bool bface) {
  ...
  PT(StandardMaterial) mat = new StandardMaterial(egg_mat->get_name());

  if (egg_mat->has_base()) {
    mat->set_base_color(egg_mat->get_base());

  } else if (egg_mat->has_diff()) {
    // Diffuse is also accepted as base color if actual base color is not there.
    mat->set_base_color(egg_mat->get_diff());
  }

  if (egg_mat->has_emit()) {
    mat->set_emission_enabled(true);
    mat->set_emission(1.0f);
    mat->set_emission_tint(egg_mat->get_emit().get_xyz());
  }
  ...
  if (egg_mat->has_metallic()) {
    mat->set_metalness(egg_mat->get_metallic());
  }

  CPT(RenderAttrib) ma = MaterialAttrib::make(mat);
  materials.insert(Materials::value_type(egg_mat, ma));
