...
#ifndef _X360
// Using STDC or SSE
ALWAYS_INLINE fltx4 LoadAlignedSIMD( const LQuaternion & pSIMD )
{
	fltx4 retval = LoadAlignedSIMD( pSIMD.get_data() );
	return retval;
}
...
ALWAYS_INLINE void StoreAlignedSIMD( LQuaternion *  pSIMD, const fltx4 & a )
{
	StoreAlignedSIMD( (float *)pSIMD->get_data(), a );
}
#else

// for the transitional class -- load a LQuaternion
ALWAYS_INLINE fltx4 LoadAlignedSIMD( const LQuaternion & pSIMD )
{
	fltx4 retval = XMLoadVector4A( pSIMD.get_data() );
	return retval;
}
...
ALWAYS_INLINE void StoreAlignedSIMD( LQuaternion *  pSIMD, const fltx4 & a )
{
	XMStoreVector4A( pSIMD->get_data(), a );
}
...
ALWAYS_INLINE fltx4 QuaternionScaleSIMD( const fltx4 &p, const fltx4 &t4 )
{
	fltx4 sinom = Dot3SIMD( p, p );
	sinom = SqrtSIMD( sinom );
	sinom = MinSIMD( sinom, Four_Ones );
...
class EXPCL_PANDA_SSEMATH ALIGN_16BYTE FourQuaternions
{
public:
	fltx4 x,y,z,w;
	...
	ALWAYS_INLINE void operator=( FourQuaternions const &src )
	{
		x=src.x;
		y=src.y;
		z=src.z;
	...
	/// this = this * q;
	ALWAYS_INLINE FourQuaternions Mul( FourQuaternions const &q ) const;

	/// negate the vector part
	ALWAYS_INLINE FourQuaternions Conjugate() const;

	/// for a quaternion representing a rotation of angle theta, return
	/// one of angle s*theta
	/// scale is four floats -- one for each quat
	ALWAYS_INLINE FourQuaternions ScaleAngle( const fltx4 &scale ) const;
	...
	ALWAYS_INLINE FourQuaternions MulAc( const fltx4 &s, const FourQuaternions &q ) const;

	/// ret = ( s * this ) * q
	ALWAYS_INLINE FourQuaternions ScaleMul( const fltx4 &s, const FourQuaternions &q ) const;

	/// Slerp four quaternions at once, FROM me TO the specified out.
	ALWAYS_INLINE FourQuaternions Slerp( const FourQuaternions &to, const fltx4 &t );

	ALWAYS_INLINE FourQuaternions SlerpNoAlign( const FourQuaternions &originalto, const fltx4 &t );

  ALWAYS_INLINE FourQuaternions Blend(const FourQuaternions &to, const fltx4 &t );

  ALWAYS_INLINE FourQuaternions BlendNoAlign(const FourQuaternions &to, const fltx4 &t);
	...
	ALWAYS_INLINE FourQuaternions &FromAxisAndAngles( const fltx4 &axis,
		const float &angle0, const float &angle1,	const float &angle2, const float &angle3 );
	ALWAYS_INLINE FourQuaternions &FromAxisAndAngles( const fltx4 &axis, const fltx4 &angles );
	// one convenience imp if you're doing this in degrees
	ALWAYS_INLINE FourQuaternions &FromAxisAndAnglesInDegrees( const fltx4 &axis, const fltx4 &angles )
	{
	...
	ALWAYS_INLINE void RotateFourVectors( FourVectors *  vecs ) const  ;


	/// LoadAndSwizzleAligned - load 4 LQuaternions into a FourQuaternions, performing transpose op.
	/// all 4 vectors must be 128 bit boundary
	ALWAYS_INLINE void LoadAndSwizzleAligned(const float * a, const float * b, const float * c, const float * d)
	{
#if defined( _X360 )
	...
	ALWAYS_INLINE void LoadAndSwizzleAligned(const LQuaternion *  a,
		const LQuaternion *  b,
		const LQuaternion *  c,
		const LQuaternion *  d)
	{
	...
	/// LoadAndSwizzleAligned - load 4 consecutive LQuaternions into a FourQuaternions,
	/// performing transpose op.
	/// all 4 vectors must be 128 bit boundary
	ALWAYS_INLINE void LoadAndSwizzleAligned(const LQuaternion *qs)
	{
#if defined( _X360 )
	...
	ALWAYS_INLINE void SwizzleAndStoreAligned(LQuaternion *a, LQuaternion *b, LQuaternion *c, LQuaternion *d)
	{
#if defined( _X360 )
	...
	ALWAYS_INLINE void SwizzleAndStoreAligned(LQuaternion *qs)
	{
#if defined( _X360 )
		fltx4 r0 = __vmrghw(x, z);
		fltx4 r1 = __vmrghw(y, w);
	...
	ALWAYS_INLINE void SwizzleAndStoreAlignedMasked(LQuaternion *  qs, const bi32x4 &controlMask)
	{
		fltx4 originals[4];
...
ALWAYS_INLINE FourQuaternions FourQuaternions::Conjugate(  ) const
{
...
ALWAYS_INLINE const fltx4 Dot(const FourQuaternions &a, const FourQuaternions &b)
{
	return
		MaddSIMD(a.x, b.x,
			MaddSIMD(a.y, b.y,
...
ALWAYS_INLINE const FourQuaternions Madd(const FourQuaternions &a, const fltx4 &scale, const FourQuaternions &c)
{
	FourQuaternions ret;
...
ALWAYS_INLINE const FourQuaternions Mul(const FourQuaternions &a, const fltx4 &scale)
{
	FourQuaternions ret;
...
ALWAYS_INLINE const FourQuaternions Add(const FourQuaternions &a,const FourQuaternions &b)
{
	FourQuaternions ret;
...
ALWAYS_INLINE const FourQuaternions Sub(const FourQuaternions &a,const FourQuaternions &b)
{
	FourQuaternions ret;
...
ALWAYS_INLINE const FourQuaternions Neg(const FourQuaternions &q)
{
	FourQuaternions ret;
	ret.x = NegSIMD(q.x);
	ret.y = NegSIMD(q.y);
...
ALWAYS_INLINE const FourQuaternions MaskedAssign(const bi32x4 &mask, const FourQuaternions &a, const FourQuaternions &b)
{
	FourQuaternions ret;
...
ALWAYS_INLINE const FourQuaternions MaskedAssign(const fltx4 &mask, const FourQuaternions &a, const FourQuaternions &b)
{
...
ALWAYS_INLINE FourQuaternions QuaternionAlign( const FourQuaternions &p, const FourQuaternions &q )
{
	// decide if one of the quaternions is backwards
...
ALWAYS_INLINE const FourQuaternions QuaternionNormalize( const FourQuaternions &q )
{
...
#if !defined(__SPU__)
ALWAYS_INLINE FourQuaternions &FourQuaternions::FromAxisAndAngles( const fltx4 &axis,
											   const float &angle0, const float &angle1,	const float &angle2, const float &angle3 )
{
...
/// this = this * q;
ALWAYS_INLINE FourQuaternions FourQuaternions::Mul( FourQuaternions const &q ) const
{
...
ALWAYS_INLINE void FourQuaternions::RotateFourVectors( FourVectors *  vecs ) const
{
...
ALWAYS_INLINE FourQuaternions FourQuaternions::ScaleAngle( const fltx4 &scale ) const
{
	FourQuaternions ret;
...
ALWAYS_INLINE FourQuaternions FourQuaternions::MulAc( const fltx4 &s, const FourQuaternions &q ) const
{
	/*
...
ALWAYS_INLINE FourQuaternions FourQuaternions::ScaleMul( const fltx4 &s, const FourQuaternions &q ) const
{
	return ScaleAngle(s).Mul(q);
}


ALWAYS_INLINE FourQuaternions FourQuaternions::Slerp( const FourQuaternions &originalto, const fltx4 &t )
{
	FourQuaternions ret;
	static const fltx4 OneMinusEpsilon = {1.0f -  0.000001f, 1.0f -  0.000001f, 1.0f -  0.000001f, 1.0f -  0.000001f };
...
ALWAYS_INLINE FourQuaternions FourQuaternions::SlerpNoAlign( const FourQuaternions &originalto, const fltx4 &t )
{
	FourQuaternions ret;
	static const fltx4 OneMinusEpsilon = {1.0f -  0.000001f, 1.0f -  0.000001f, 1.0f -  0.000001f, 1.0f -  0.000001f };
...
ALWAYS_INLINE FourQuaternions FourQuaternions::
Blend(const FourQuaternions &to, const fltx4 &t) {
  FourQuaternions q = QuaternionAlign(*this, to);
  return q.BlendNoAlign(to, t);
}

ALWAYS_INLINE FourQuaternions FourQuaternions::
BlendNoAlign(const FourQuaternions &to, const fltx4 &sclq) {
  fltx4 sclp = SubSIMD( Four_Ones, sclq );

  FourQuaternions ret;

  ret.x = MulSIMD(sclp, x);
  ret.x = MaddSIMD(sclq, to.x, ret.x);

  ret.y = MulSIMD(sclp, y);
  ret.y = MaddSIMD(sclq, to.y, ret.y);

  ret.z = MulSIMD(sclp, z);
  ret.z = MaddSIMD(sclq, to.z, ret.z);

  ret.w = MulSIMD(sclp, w);
  ret.w = MaddSIMD(sclq, to.w, ret.w);

	return QuaternionNormalize( ret );
}
