...
const int32_t ALIGN_16BYTE g_SIMD_clear_signmask[4] = {0x7fffffff, 0x7fffffff, 0x7fffffff, 0x7fffffff};
const uint32_t ALIGN_16BYTE g_SIMD_signmask[4] = {0x80000000, 0x80000000, 0x80000000, 0x80000000};
const uint32_t ALIGN_16BYTE g_SIMD_lsbmask[4] = {0xfffffffe, 0xfffffffe, 0xfffffffe, 0xfffffffe};
...
const uint32_t ALIGN_16BYTE g_SIMD_SkipTailMask[4][4] =
    {
...
void FourVectors::RotateManyBy(FourVectors *pVectors, unsigned int numVectors, const LMatrix4 &rotationMatrix)
{
        nassertv(numVectors > 0);
        if (numVectors == 0)
                return;
...

        {
                ...
                fltx4 matCol0 = LoadUnalignedSIMD(rotationMatrix.get_col(0).get_data());
                fltx4 matCol1 = LoadUnalignedSIMD(rotationMatrix.get_col(1).get_data());
                fltx4 matCol2 = LoadUnalignedSIMD(rotationMatrix.get_col(2).get_data());
